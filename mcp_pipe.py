...
async def connect_to_server(uri, target):
    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            # If the child process exits, force reconnect by surfacing an exception.
            await asyncio.gather(
                ...
                monitor_process_exit(process, target)
            )
...
async def monitor_process_exit(process, target):
    """Wait for child process exit and trigger reconnect logic."""
    return_code = await asyncio.to_thread(process.wait)
    raise RuntimeError(f"[{target}] Server process exited with code {return_code}")
